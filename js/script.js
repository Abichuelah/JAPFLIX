document.addEventListener('DOMContentLoaded', () => {
            ...
            li.innerHTML = `
                ...
                <button id="${buttonId}" class="list-group-item list-group-item-action bg-dark text-light" type="button" data-bs-toggle="offcanvas" data-bs-target="#${offcanvasId}" aria-controls="${offcanvasId}">
                    <div class="d-flex w-100 justify-content-between">
                        <h5 class="mb-1 fw-bold">${pelicula.title}</h5> <!-- Título normal -->
                        <small>${generarEstrellas(pelicula.vote_average)}</small> <!-- Estrellas con nuevos colores -->
                    </div>
                ...
                <div class="offcanvas offcanvas-top" tabindex="-1" id="${offcanvasId}" aria-labelledby="offcanvasTopLabel-${pelicula.id}">
                    <div class="offcanvas-header">
                        <div class="col">
                            <h5 class="offcanvas-title fs-2" id="offcanvasTopLabel-${pelicula.id}" style="color: blue;">${pelicula.title}</h5> <!-- Título del offcanvas en color azul -->
    ...
    function generarEstrellas(voteAverage) {
        const estrellitas = Math.round(voteAverage / 2); // Convierte de escala de 10 a 5
        let estrellitasHTML = '';
        for (let i = 0; i < 5; i++) {
            if (i < estrellitas) {
                estrellitasHTML += '<i class="fa fa-star text-warning-red" style="color: red;"></i>'; // Estrella rellena de color rojo
            } else {
                estrellitasHTML += '<i class="fa fa-star-o text-warning-red" style="color: red;"></i>'; // Estrella vacía contorno rojo
            }
        }
        return estrellitasHTML;
    }
